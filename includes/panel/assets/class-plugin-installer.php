...
// Start Class
class Wishful_Blog_Extra_Demo_Import_Plugin_Installer {
    ...
    public static function init($plugins) {
        ...
        foreach ($plugins as $plugin) :
            ...
            $button_text = __('Install Now', 'wishful-companion');
            ...
            if (!is_wp_error($api)) { // confirm error free
                $main_plugin_file = Wishful_Blog_Extra_Demo_Import_Plugin_Installer::get_plugin_file($plugin['slug']); // Get main plugin file

                if (self::check_file_extension($main_plugin_file)) { // check file extension
                    if (is_plugin_active($main_plugin_file)) {
                        // plugin activation, confirmed!
                        $button_classes = 'button disabled';
                        $button_text = __('Activated', 'wishful-companion');
                    } else {
                        // It's installed, let's activate it
                        $button_classes = 'activate button button-primary';
                        $button_text = __('Activate', 'wishful-companion');
                    }
                }
    ...
    public static function render_template($plugin, $api, $button_text, $button_classes) {
        ...
        <div class="plugin">
            <div class="plugin-wrap">
                ...
                <p class="plugin-author"><?php esc_html_e('By', 'wishful-companion'); ?> <?php echo esc_html($api->author); ?></p>
            </div>
            ...
            <ul class="activation-row">
                ...
                <li>
                    <a href="https://wordpress.org/plugins/<?php echo esc_attr($api->slug); ?>/" target="_blank"><?php esc_html_e('More Details', 'wishful-companion'); ?></a>
                </li>
            </ul>
        </div>
    ...
    public static function render_premium_template($plugin, $api, $button_text, $button_classes) {
        ...
        <div class="plugin">
            <div class="plugin-wrap">
                ...
                if ($author) {
                    ?>
                    <p class="plugin-author"><?php esc_html_e('By', 'wishful-companion'); ?> <a href="<?php echo esc_url($author_url); ?>"><?php echo esc_html($author); ?></a></p>
                <?php }
                ?>
            </div>
            ...
            <ul class="activation-row">
                <li>
                    ...
                    $main_plugin_file = Wishful_Blog_Extra_Demo_Import_Plugin_Installer::get_plugin_file($plugin['slug']);
                    ...
                    if (self::check_file_extension($main_plugin_file)) {
                        ...
                        if ($full_url) {
                            ?>
                            <a class="button premium-link" href="<?php echo esc_url($full_url); ?>" target="_blank"><?php esc_html_e('Get This Plugin', 'wishful-companion'); ?></a>
                            <?php
                        }
                    }
                    ?>
                ...
                <li>
                    ...
                    if ($full_url) {
                        ?>
                        <a href="<?php echo esc_url($full_url); ?>" target="_blank"><?php esc_html_e('More Details', 'wishful-companion'); ?></a>
                        <?php
                    }
                    ?>
                </li>
                <li class="ribbon">
                    <?php esc_html_e('Premium', 'wishful-companion'); ?>
                </li>
            </ul>
        </div>
    ...
    public function oe_plugin_installer() {
        ...
        if (!current_user_can('install_plugins')) {
            wp_die(esc_html__('Sorry, you are not allowed to install plugins on this site.', 'wishful-companion'));
        }
        ...
        if (!wp_verify_nonce($nonce, 'oe_installer_nonce')) {
            wp_die(esc_html__('Error - unable to verify nonce, please try again.', 'wishful-companion'));
        }
    ...
    public function oe_plugin_activation() {
        ...
        if (!current_user_can('install_plugins')) {
            wp_die(esc_html__('Sorry, you are not allowed to activate plugins on this site.', 'wishful-companion'));
        }
        ...
        // Check our nonce, if they don't match then bounce!
        if (!wp_verify_nonce($nonce, 'oe_installer_nonce')) {
            die(esc_html__('Error - unable to verify nonce, please try again.', 'wishful-companion'));
        }
        ...
        if ($api->name) {
            $main_plugin_file = Wishful_Blog_Extra_Demo_Import_Plugin_Installer::get_plugin_file($plugin);
            $status = 'success';
            if ($main_plugin_file) {
    ...
    public static function enqueue_scripts($hook) {
        ...
        if (WISHFUL_COMPANION_ADMIN_PANEL_HOOK_PREFIX . '-extensions' != $hook && WISHFUL_COMPANION_ADMIN_PANEL_HOOK_PREFIX . '-rec-plugins' != $hook) {
            return;
        }

        // JS
        wp_enqueue_script('wishful-companion-installer', plugins_url('/js/installer.js', __FILE__), array('jquery'));

        wp_localize_script('wishful-companion-installer', 'oe_installer_localize', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'admin_nonce' => wp_create_nonce('oe_installer_nonce'),
            'install_now' => __('Are you sure you want to install this plugin?', 'wishful-companion'),
            'install_btn' => __('Install Now', 'wishful-companion'),
            'activate_btn' => __('Activate', 'wishful-companion'),
            'installed_btn' => __('Activated', 'wishful-companion')
        ));

        // CSS
        wp_enqueue_style('wishful-companion-installer', plugins_url('/css/installer.css', __FILE__));
    }

}

// initialize
$Wishful_Blog_Extra_Demo_Import_Plugin_Installer = new Wishful_Blog_Extra_Demo_Import_Plugin_Installer();
$Wishful_Blog_Extra_Demo_Import_Plugin_Installer->start();
